...
def send_email_brevo(subject, plain_message, html_message, to_emails):
    ...
    if not brevo_key:
        if _has_brevo_smtp_config():
            ...
            logger.warning(
                'Brevo email is not configured and no Brevo SMTP relay is available. '
                'DEBUG mode is active, so email content will be emitted to the terminal via the console backend.'
            )
            ...
            logger.info('Debug email output to terminal for %s', to_emails)
            return
        else:
