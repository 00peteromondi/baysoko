...
def _attach_image_urls_to_product(product, image_urls):
    """Attach images from URLs to product. Silently skips failed downloads and validations."""
    ...
    for index, image_url in enumerate(image_urls, start=1):
        try:
            ...
            if not img_bytes or not validate_image_bytes(img_bytes):
                logger.debug('Image validation failed for URL %s', image_url)
                continue
            ...
            if not listing_image:
                logger.debug('Failed to save listing image for product %s', product.pk)
                continue
            ...
            logger.debug('Failed to attach image from %s: %s', image_url, e)
            continue

    # No error raised if no images attached — allows import to continue
    if not attached_any:
        logger.info('No images successfully attached from provided URLs for product %s', product.pk)
...
def process_import_task(self, job_id):
    ...
    try:
        ...
        params = job.parameters
        skip_errors = params.get('skip_errors', False)
