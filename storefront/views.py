...
def _listing_form_context(request, form, store=None, user_stores=None):
    stores = user_stores if user_stores is not None else Store.objects.filter(owner=request.user)
    categories = Category.objects.filter(is_active=True)
    category_schemas = {}
    try:
        cats = categories.only('id', 'fields_schema', 'schema_group')
        group_map = {}
        for category in cats:
            if category.schema_group and category.fields_schema:
                group_map[category.schema_group] = category.fields_schema
        for category in cats:
            key = str(category.id)
            if category.fields_schema:
                category_schemas[key] = category.fields_schema
            elif category.schema_group and category.schema_group in group_map:
                category_schemas[key] = group_map[category.schema_group]
            else:
                category_schemas[key] = {}
    except Exception:
        logger.exception('Failed to build listing category schemas for storefront form')
        category_schemas = {}

    try:
        initial_dynamic_fields = form.instance.dynamic_fields if getattr(form.instance, 'dynamic_fields', None) else {}
    except Exception:
        initial_dynamic_fields = {}

    try:
        seller_ai_access = has_seller_ai_access(request.user, store=store or stores)
    except Exception:
    ...
    return {
        ...
        'initial_dynamic_fields': initial_dynamic_fields,
        'seller_ai_copilot_access': seller_ai_access,
        'location_choices': list(Listing.HOMABAY_LOCATIONS),
    }
...
def product_create(request, store_slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('storefront:store_detail', slug=store.slug)
        messages.error(request, "Please fix the highlighted fields before saving your listing.")
        context = _listing_form_context(request, form, store=store, user_stores=user_stores)
        # Add error flag to help form retain data
    ...
    return render(request, 'listings/listing_form.html', _listing_form_context(request, form, store=store, user_stores=user_stores))
