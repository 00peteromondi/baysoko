...
class Listing(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Generate or normalize slug before saving
        from django.utils.text import slugify
        if not self.slug:
            base_slug = slugify(self.title) or 'listing'
            self.slug = base_slug
        else:
            self.slug = slugify(self.slug) if self.slug else self.slug
            base_slug = self.slug or 'listing'
        ...
        while True:
            try:
                ...
                message = str(exc)
                if 'listings_listing.slug' in message or 'UNIQUE constraint failed: listings_listing.slug' in message:
                    save_attempts += 1
                    if save_attempts > 5:
                        raise
                    self.slug = f"{base_slug}-{counter}"
