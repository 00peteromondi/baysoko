...
class ListingCreatePermissionTests(TestCase):
    ...
    def test_listing_create_with_duplicate_title_generates_unique_slug(self):
        user = self.create_verified_user('seller5', password='pass5')
        self.client.login(username='seller5', password='pass5')
        store = Store.objects.create(owner=user, name='Store5', slug='store5')
        ...
        self.assertEqual(first_listing.slug, 'duplicate-title')

        image = SimpleUploadedFile('duplicate.jpg', b'testimagecontent', content_type='image/jpeg')
        ...
        if response.status_code != 302:
            ...
            print('RESPONSE TEMPLATES', [t.name for t in response.templates])
            if response.context and response.context.get('form'):
                print('FORM ERRORS', response.context['form'].errors)
                print('FORM NON-FIELD ERRORS', response.context['form'].non_field_errors())
            print(response.content.decode('utf-8')[:2000])

        self.assertEqual(response.status_code, 302)
        second_listing = Listing.objects.get(title='Duplicate Title', slug__startswith='duplicate-title-')
        self.assertNotEqual(second_listing.slug, first_listing.slug)
        self.assertTrue(second_listing.slug.startswith('duplicate-title-'))
    ...
    def test_invalid_listing_post_returns_full_context_and_field_errors(self):
        ...
        self.assertContains(response, 'Please fix the highlighted fields before saving your listing.', status_code=400)
