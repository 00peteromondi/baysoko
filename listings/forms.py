...
class ListingForm(forms.ModelForm):
    ...
    def clean_image(self):
        ...
        # If the form files include an image upload, accept it even if cleaned_data
        # doesn't yet contain it (defensive for multipart parsing edge-cases).
        has_uploaded_image = False
        try:
            if hasattr(self, 'files') and getattr(self.files, 'get', None):
    ...
    def clean_video_descriptions(self):
        ...
        try:
            if hasattr(self.files, 'getlist'):
                ...
                raw = self.files.get('video_descriptions')
                if raw is None:
                    videos = []
